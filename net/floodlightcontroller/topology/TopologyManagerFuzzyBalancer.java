/**
 *
 * This is an extended, modified version of the original TopologyManager
...
import ru.sut.fuzzybalancer.IFuzzyBalancerListener;
import ru.sut.fuzzybalancer.IFuzzyBalancerService;

@LogMessageCategory("Network Topology")
public class TopologyManagerFuzzyBalancer extends ServerResource implements
		IFloodlightModule, ITopologyService, IRoutingService,
		ILinkDiscoveryListener, IOFMessageListener, IHAListener,
		IFuzzyBalancerListener {

	protected static Logger log = LoggerFactory
			.getLogger(TopologyManagerFuzzyBalancer.class);
	...
	protected TopologyInstanceFuzzyBalancer currentInstance;
	protected TopologyInstanceFuzzyBalancer currentInstanceWithoutTunnels;

	protected SingletonTask newInstanceTask;
	private Date lastUpdateTime;

	/**
	 * Flag that indicates if links (direct/tunnel/multihop links) were updated
	 * as part of LDUpdate.
	 */
	...
	/**
	 * Thread for recomputing topology. The thread is always running, however
	 * the function applyUpdates() has a blocking call.
	 */
	...
	public boolean isAttachmentPointPort(long switchid, short port,
		...
		TopologyInstanceFuzzyBalancer ti = getCurrentInstance(tunnelEnabled);
	...
	// //////////////////////////////////////////////////////////////////////
	/**
	 * Get all the ports on the target switch (targetSw) on which a broadcast
	 * packet must be sent from a host whose attachment point is on switch port
	 * (src, srcPort).
	 */
	...
	/**
	 * Checks if the new attachment point port is consistent with the old
	 * attachment point port.
	 */
	@Override
	public boolean isConsistent(long oldSw, short oldPort, long newSw,
			short newPort) {
		return isConsistent(oldSw, oldPort, newSw, newPort, true);
	}
	...
	public NodePortTuple getAllowedOutgoingBroadcastPort(long src,
		...
		return getAllowedOutgoingBroadcastPort(src, srcPort, dst, dstPort, true);
	}
	...
	public NodePortTuple getAllowedOutgoingBroadcastPort(long src,
		...
		TopologyInstanceFuzzyBalancer ti = getCurrentInstance(tunnelEnabled);
		return ti.getAllowedOutgoingBroadcastPort(src, srcPort, dst, dstPort);
	}
	...
	public void roleChanged(Role oldRole, Role newRole) {
		...
		case MASTER:
			...
			break;
		case SLAVE:
			log.debug("Clearing topology due to " + "HA change to SLAVE");
	...
	public void init(FloodlightModuleContext context)
		...
		mpbalance = context.getServiceImpl(IFuzzyBalancerService.class);
	 ...
	 * @return
	 */
	protected Command dropFilter(long sw, OFPacketIn pi, FloodlightContext cntx) {
	...
	/**
	 * TODO This method must be moved to a layer below forwarding so that anyone
	 * can use it.
	 * 
	 * @param packetData
	 * @param sw
	...
	public void doMultiActionPacketOut(byte[] packetData, IOFSwitch sw,
		...
		// set action length
		po.setActionsLength((short) (OFActionOutput.MINIMUM_LENGTH * ports
				.size()));
	 ...
	 * openflowdomain. Get all the switches in the same openflow domain as the
	 * sw (disabling tunnels). Then get all the external switch ports and send
	 * these packets out.
	 * 
	 * @param sw
	 * @param pi
	...
	protected void doFloodBDDP(long pinSwitch, OFPacketIn pi,
		...
		TopologyInstanceFuzzyBalancer ti = getCurrentInstance(false);
		...
		while (ldUpdates.peek() != null) {
			...
			if (update.getOperation() == UpdateOperation.LINK_UPDATED) {
				addOrUpdateLink(update.getSrc(), update.getSrcPort(),
						update.getDst(), update.getDstPort(), update.getType());
	 ...
	 */
	/**
	 * This function computes a new topology instance. It ignores links
	 * connected to all broadcast domain ports and tunnel ports. The method
	 * returns if a new instance of topology was created or not.
	 */
	protected boolean createNewInstance() {
		...
		TopologyInstanceFuzzyBalancer nt = new TopologyInstanceFuzzyBalancer(
				switchPorts, blockedPorts, openflowLinks,
				portBroadcastDomainLinks.keySet(), tunnelLinks.keySet(),
				mpbalance);
		nt.compute();
	...
	public boolean removeSwitchPort(long sw, short port) {
		...
		if (switchPortLinks.containsKey(npt) == false)
			return false;
	...
	public void removeLink(Link link) {
		...
		NodePortTuple srcNpt = new NodePortTuple(link.getSrc(),
				link.getSrcPort());
		NodePortTuple dstNpt = new NodePortTuple(link.getDst(),
				link.getDstPort());
	...
	public TopologyInstanceFuzzyBalancer getCurrentInstance(
			boolean tunnelEnabled) {
		if (tunnelEnabled)
	...
	public void costChanged() {
		try {
			boolean result = createNewCostInstance();
			if (result == true)
				log.trace("Topology update Success");
			else
				log.trace("Topology update Failed");
		} catch (Exception e) {
			log.error("Error in topology instance task thread", e);
		}
	...
	protected boolean createNewCostInstance() throws IOException {
		Set<NodePortTuple> blockedPorts = new HashSet<NodePortTuple>();

		Map<NodePortTuple, Set<Link>> openflowLinks = new HashMap<NodePortTuple, Set<Link>>(
				switchPortLinks);
